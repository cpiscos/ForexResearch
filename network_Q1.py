...
save = True
load = True
network_file = 'saves/network_Q1.pt'
graph_file = 'saves/graph_Q1.pkl'
batch_size = 2000
epochs = 20
cycles = 1
seq_len = 200
...
train = df.iloc[-80302 - seq_len - pred_hours:-372, 1:]
...
def gain(pred, pip):
    ...
    return torch.mean((pred[:,:2] * convert).t() * pip) * 2 + torch.mean(pred[:,2]) * 0.0001
...
class ForexLSTM(nn.Module):
    ...
    def forward(self, input):
        ...
        # out = out+self.scaled
        self.out = out[:, -1, :]
        self.confidence = self.sigmoid(self.linear(self.out).reshape(-1,3))

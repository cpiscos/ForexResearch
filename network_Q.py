...
save = True
load = True
network_file = 'saves/network_Q.pt'
graph_file = 'saves/graph_Q.pkl'
batch_size = 2000
epochs = 15
cycles = 3
seq_len = 50
...
# df = np.concatenate((df**2, df), axis=1)
pred_hours = 1 - 1
# -8302 - seq_len - pred_hours   -25302 - seq_len - pred_hours
train = df[:-372]
test = df[-372 - seq_len - pred_hours:]

convert = torch.DoubleTensor([1, -1]).to(device)
convert.requires_grad = True
def gain(pred, pip):
    # torch.nn.functional.tanh(pip)
    return torch.mean((pred * convert).t() * torch.nn.functional.tanh(pip))
...
def profit(pred, pip, total=True, accuracy=False):
    ...
    ned = torch.where(ned == 2, zero, ned)
    ned = ned.cpu().numpy()
    pip = pip.cpu().numpy()
    ap = ned * pip * 10000
    if total:
        ...
        accuracy = np.mean(ned == np.where(pip > 0, 1, -1))
        return accuracy
    else:
        return ap
...
class Forex(Dataset):
    ...
    def __getitem__(self, item):
        ...
        self.a_diff = self.df[item + self.seq_len + self.pred_hours] - self.df[item + self.seq_len - 1]
        # pips = a_diff * self.df[item + self.seq_len - 1]
        norm = raw[1:] - raw[:-1]
        self.tposed = np.concatenate((np.zeros(norm.shape[1])[None], norm), axis=0) #/ std
        return self.tposed, self.a_diff[-2]
...
class ForexLSTM(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(65, 65, num_layers=2, batch_first=True)
    ...
    def forward(self, input):
        ...
        # out = out+self.scaled
        self.out = out[:, -1, :]
        return self.sigmoid(self.linear(self.out).reshape(-1,2))
...
test_loader = DataLoader(dataset=test_dataset, batch_size=10000, shuffle=False, num_workers=0, pin_memory=True)
        ...
        with torch.no_grad():
            ...
            for param in optimizer.param_groups:
                dict = {'epoch': [f'{epoch+1}/{epochs}'],
                        ...
                        'Batch profit': [np.mean(np.where(np.array(batch_profit)>0,1,0))],
